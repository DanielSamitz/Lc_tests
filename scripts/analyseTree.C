...
Float_t fDecayLength, fDecayLengthXY, fPtProng0, fPProng0, fPtProng1, fImpactParameter0, fImpactParameter1, fV0Radius, fV0CosPA, fV0MLambda, fV0MAntiLambda, fV0MK0Short, fV0MGamma, fV0CtK0Short,fV0CtLambda, fDCAV0Daughters, fPtV0Pos, fPtV0Neg, fDCANegToPV, fDCAPosToPV, fNSigmaTPCPr0, fNSigmaTOFPr0, fM, fPt, fCPA, fCPAXY, fCt, fEta, fPhi, fY, fE;
Char_t fFlagMc;
...
int set_cut(float cut[2], const char* name, const char* name1, const char* name2, nlohmann::json this_json){
    if (!this_json.contains(name)) {
      printf("ERROR: Could not find %s\n",name);
      return 1;
   }
...
int set_cut(bool cut[2], const char* name, const char* name1, const char* name2, nlohmann::json this_json){
    if (!this_json.contains(name)) {
      printf("ERROR: Could not find %s\n",name);
      return 1;
   }
...
int set_cut(std::vector<float> &cut, const char* name, const char* name1, nlohmann::json this_json){
    if (!this_json.contains(name)) {
      printf("ERROR: Could not find %s\n",name);
      return 1;
   }
...
int set_cut(std::vector<float> &cut, int which, const char* name, const char* name1, nlohmann::json this_json){
    if (!this_json.contains(name)) {
      printf("ERROR: Could not find %s\n",name);
      return 1;
   }
...
int read_json(TString config_file){
   ...
   nlohmann::json configfile = configfile_toplevel["hf-candidate-selector-lc-to-k0s-p"];
   if (set_cut(cut_Pid_threshold,"pPidThreshold",configfile)!=0) return 1;
   ...
   if (set_cut(cut_Tpc_lowP,"nSigmaTpcMaxLowP",configfile)!=0) return 1;
   if (set_cut(cut_Tpc_highP,"nSigmaTpcMaxHighP",configfile)!=0) return 1;
   if (set_cut(cut_Tof_lowP,"nSigmaTofMaxLowP",configfile)!=0) return 1;
   if (set_cut(cut_Tof_highP,"nSigmaTofMaxHighP",configfile)!=0) return 1;
   if (set_cut(cut_ReqTof_lowP,"requireTofLowP",configfile)!=0) return 1;
   if (set_cut(cut_ReqTof_highP,"requireTofHighP",configfile)!=0) return 1;
   if (set_cut(cut_v0Radius,"v0RadiusMin",configfile)!=0) return 1;
   if (set_cut(cut_v0DaugDCAToPV,"v0DauDCAToPVMin",configfile)!=0) return 1;
   if (set_cut(cut_v0DauDCA,"v0DauDCAMax",configfile)!=0) return 1;
   if (set_cut(cut_v0CosPA,"v0CosPAMin",configfile)!=0) return 1;
   if (set_cut(doMc,"processMc",configfile)!=0) return 1;
   if (set_cut(ptBins,"binsPt","values",configfile)!=0) return 1;
   if (set_cut(cut_mK0s,0,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_mLambda,1,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_mGamma,2,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_ptBach,3,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_ptV0dau,4,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_ptV0,5,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_d0Bach,6,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_d0V0,7,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_decLengthMin,8,"cuts","values",configfile)!=0) return 1;
   if (set_cut(cut_decLengthMax,9,"cuts","values",configfile)!=0) return 1;
...
void select(uint32_t &status){
   ...
   if ((std::abs(fV0MLambda - getMassPDG(kLambda0)) < cut_mLambda[ptBin]) || (std::abs(fV0MAntiLambda - getMassPDG(kLambda0)) < cut_mLambda[ptBin])) {
   ...
   if ((std::abs(fDCANegToPV) < cut_v0DaugDCAToPV) || (std::abs(fDCAPosToPV) < cut_v0DaugDCAToPV)) {
...
int analyseTree(TString input_files, TString config_file, TString output_file = "AnalysisResults_tree.root"){
   ...
   TH1F* th1bg[nHistos];
   TH2F* th2bg[nHistos];
   TString histnames[nHistos]={"hEtaCand","hPhiCand","hMass","hPtBach","hPtV0","hd0Bach","hd0V0","hd0V0pos","hd0V0neg","hPtV0pos","hPtV0neg","hV0CPA","hV0Radius","hV0DCADaughters","hV0MK0Short","hV0MLambda","hV0MAntiLambda","hV0Gamma","hCtV0K0Short","hCtV0Lambda","hCPACand","hCPAxyCand","hDecLengthCand","hDecLengthXYCand","hCtCand"};
   ...
   for (int i=0; i<nHistos; i++){
      ...
      TH2F* temp2 = new TH2F(histnames[i]+"VsPtCand",histnames[i]+"VsPtCand",1,0,1,1,0,1);
      make_axis(temp1,axes[i]);
      make_axis(temp2,axes[i]);
      th1[i]=temp1;
      th2[i]=temp2;
      if (doMc){
         TH1F* temp1sig = new TH1F(histnames[i]+"RecSig",histnames[i]+"RecSig",1,0,1);
         TH2F* temp2sig = new TH2F(histnames[i]+"VsPtCand"+"RecSig",histnames[i]+"VsPtCand"+"RecSig",1,0,1,1,0,1);
         TH1F* temp1bg = new TH1F(histnames[i]+"RecBg",histnames[i]+"RecBg",1,0,1);
         TH2F* temp2bg = new TH2F(histnames[i]+"VsPtCand"+"RecBg",histnames[i]+"VsPtCand"+"RecBg",1,0,1,1,0,1);
   ...
   TH1F* hTPCNSigmaPrBach = new TH1F("hTPCNSigmaPrBach", "hTPCNSigmaPrBach", 1, 0, 1);
   TH2F* hPBachVsTPCNSigmaPrBach = new TH2F("hPBachVsTPCNSigmaPrBach", "hPBachVsTPCNSigmaPrBach", 1, 0, 1,1,0,1);
   TH1F* hTOFNSigmaPrBach = new TH1F("hTOFNSigmaPrBach", "hTOFNSigmaPrBach", 1, 0, 1);
   TH2F* hPBachVsTOFNSigmaPrBach = new TH2F("hPBachVsTOFNSigmaPrBach", "hPBachVsTOFNSigmaPrBach", 1, 0, 1,1,0,1);
   ...
   TH1F* hTPCNSigmaPrBachRecSig = new TH1F("hTPCNSigmaPrBachRecSig", "hTPCNSigmaPrBachRecSig", 1, 0, 1);
   TH2F* hPBachVsTPCNSigmaPrBachRecSig = new TH2F("hPBachVsTPCNSigmaPrBachRecSig", "hPBachVsTPCNSigmaPrBachRecSig", 1, 0, 1,1,0,1);
   TH1F* hTOFNSigmaPrBachRecSig = new TH1F("hTOFNSigmaPrBachRecSig", "hTOFNSigmaPrBachRecSig", 1, 0, 1);
   TH2F* hPBachVsTOFNSigmaPrBachRecSig = new TH2F("hPBachVsTOFNSigmaPrBachRecSig", "hPBachVsTOFNSigmaPrBachRecSig", 1, 0, 1,1,0,1);
   ...
   TH1F* hTPCNSigmaPrBachRecBg = new TH1F("hTPCNSigmaPrBachRecBg", "hTPCNSigmaPrBachRecBg", 1, 0, 1);
   TH2F* hPBachVsTPCNSigmaPrBachRecBg = new TH2F("hPBachVsTPCNSigmaPrBachRecBg", "hPBachVsTPCNSigmaPrBachRecBg", 1, 0, 1,1,0,1);
   TH1F* hTOFNSigmaPrBachRecBg = new TH1F("hTOFNSigmaPrBachRecBg", "hTOFNSigmaPrBachRecBg", 1, 0, 1);
   TH2F* hPBachVsTOFNSigmaPrBachRecBg = new TH2F("hPBachVsTOFNSigmaPrBachRecBg", "hPBachVsTOFNSigmaPrBachRecBg", 1, 0, 1,1,0,1);
      ...
      while ((key = static_cast<TKey*>(next()))) {
         ...
         if (auto nextInDir = dynamic_cast<TDirectory*>(obj)){
            ...
            for (int i=0; i<nentries;i++){
               ...
               if (statusLc==0){
                  double vars[nHistos]={fEta,fPhi,fM,fPtProng0,fPtProng1,fImpactParameter0,fImpactParameter1,fDCAPosToPV,fDCANegToPV,fPtV0Pos,fPtV0Neg,fV0CosPA,fV0Radius,fDCAV0Daughters,fV0MK0Short,fV0MLambda,fV0MAntiLambda,fV0MGamma,fV0CtK0Short,fV0CtLambda,fCPA,fCPAXY,fDecayLength,fDecayLengthXY,fCt};
                  for (int i=0;i<nHistos;i++){
                     th1[i]->Fill(vars[i]);
